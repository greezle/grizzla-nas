...
- `/srv/gcode` — 512 GB SSD (`/dev/sda1` label `gcode-ssd`, fstab by UUID, `nofail`):
  ...
  `QR/` (QR sticker PNG library, tree mirrors `master/`; regenerated by the SFN
  service whenever the fleet's reference SFN table changes — the print-order
  macros link these images over the same SMB share).
